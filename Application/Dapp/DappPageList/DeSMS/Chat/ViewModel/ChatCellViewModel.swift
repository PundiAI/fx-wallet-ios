    ...
    class CellViewModel {
        ...
        init(_ sms: SmsMessage) {
            ...
            if sms.availableTimestamp > 0 {
                self.date = Date(timeIntervalSince1970: sms.availableTimestamp)
                self.isToday = self.date.isSameDay(date: CellViewModel.today)
            }
        }
        ...
        func update(status: SmsMessage.Status) {}
    }
}
...
extension ChatViewController {
    class DateCellViewModel: CellViewModel {
        ...
        override init(_ sms: SmsMessage) {
            self.dateText = Date(timeIntervalSince1970: sms.availableTimestamp).format(with: "MMM dd")
            
            super.init(sms)
            self.height = 10 + 14 + 10
        }
    }
}
...
extension ChatViewController {
    class SmsCellViewModel: CellViewModel {
        ...
        override init(_ sms: SmsMessage) {
            self.status = BehaviorRelay(value: sms.status)
            
            self.nameText = sms.isReceiver ? sms.receiveName : ""
            self.dateText = Date(timeIntervalSince1970: sms.availableTimestamp).format(with: "HH:mm")
            self.messageText = sms.message.content
        ...
        override func update(status: SmsMessage.Status) {
            ...
            self.dateText = Date(timeIntervalSince1970: rawValue.availableTimestamp).format(with: "HH:mm")
            if status != self.status.value { self.status.accept(status) }
        }
        fileprivate func parse(_ sms: TransactionMessage.SmsSendMsg) {}
    }
}
...
extension ChatViewController {
    class TextCellViewModel: SmsCellViewModel {
        ...
        override func parse(_ sms: TransactionMessage.SmsSendMsg) {
            ...
            self.calculateBubble(textSize)
            self.calculateMessage(textSize)
            self.calculateDate()
            self.calculateResend()
            self.height = self.bubbleFrame.height + 10
        }
        
        fileprivate func calculateBubble(_ textSize: CGSize) {}
        fileprivate func calculateDate() {}
        fileprivate func calculateResend() {}
        fileprivate func calculateMessage(_ textSize: CGSize) {
            self.messageFrame = CGRect(x: bubbleFrame.minX + textMargin, y: bubbleFrame.minY + bubbleMargin, width: textSize.width, height: textSize.height)
        }
    }
}
...
extension ChatViewController {
    class SenderTextCellViewModel: TextCellViewModel {
        ...
        override func calculateDate() {
            ...
            self.dateFrame = CGRect(x: bubbleFrame.minX + 14, y: bubbleFrame.maxY - bubbleMargin - dateHeight, width: size.width, height: size.height)
            self.statusFrame = CGRect(x: dateFrame.maxX, y: dateFrame.midY - 5, width: 10, height: 10)
        }
        
        override func calculateResend() {
            let size: CGFloat = 30
            self.resendFrame = CGRect(x: bubbleFrame.minX - 8 - size, y: bubbleFrame.midY - size * 0.5, width: size, height: size)
        }
    }
}
...
extension ChatViewController {
    class ReceiverTextCellViewModel: TextCellViewModel {
        ...
        override func calculateDate() {
            self.dateFrame = CGRect(x: bubbleFrame.minX + 14 , y: bubbleFrame.maxY - bubbleMargin - dateHeight, width: 70, height: dateHeight)
        }
    }
}
    ...
    class GiftCellViewModel: SmsCellViewModel {
        ...
        override func parse(_ sms: TransactionMessage.SmsSendMsg) {
            ...
            if let token = sms.transferTokens.first {
                ...
                self.tokenText = "FX"//token.denom.uppercased()
                self.tokenWidth = CGFloat(self.tokenText.count * 15)
                
                self.amountText = String(token.amount.fxc.thousandth())
