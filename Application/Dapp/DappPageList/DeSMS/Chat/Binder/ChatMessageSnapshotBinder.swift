...
extension ChatViewController {
    ...
    class SnapshotBinder {
        ...
        func show(snapshot: SnapshotProviderProtocol, inView container: UIView, onClickCopy: @escaping () -> Void,
            ...
            self.view.relayout(onClickInfo == nil)
            
            let margin: CGFloat = 18
            let actionSize = self.view.actionContainer.size
            ...
            self.view.actionContainer.origin = CGPoint(x: recommendX, y: recommendY)
            self.view.actionContainer.layer.add(animation(), forKey: "alert")
        }
        
        func hide() {
            
            self.snapshotIV?.removeFromSuperview()
