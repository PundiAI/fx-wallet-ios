...
class DappFavoriteListBinder: DappSubListBinder {
    ...
    private func bindListView() {
        ...
        listView.viewModels = { _ in
            if listViewModel.items.count > 0 {
              ...
              return  NSMutableArray.viewModels(from: [(TR("Dapp.Blank.Title"), TR("Dapp.Blank.Subtitle"))], DappBlankCell.self)
            }
        }
        
        listViewModel.refreshItems.elements.subscribe(onNext: { (xxx) in
            listView.reloadData()
        }).disposed(by: defaultBag)
