...
class SendChatGiftViewController: WKViewController {
    ...
    @objc private func selectFee(_ sender: UIButton) {
        ...
        selectedFee.value.isSelected = false
        selectedFee.accept(fee)
        selectedFee.value.isSelected = true
    }
    ...
    private func bindCommit() {
        ...
        wk.view.sendButton.rx.tap.subscribe(onNext: { [weak self] _ in
            guard let this = self,
                ...
                var amount = self?.wk.view.amountInputTF.text, amount.f > 0.00001
            else {
            ...
            Router.showAuthorizeDappAlert(dapp: .sms, types: [1]) { authVC, allow in
                Router.dismiss(authVC, animated: false) {
                    guard allow else {
                        self?.hud?.text(m: "user denied")
                        return
                    }
                    Router.showBroadcastTxAlert(tx: tx, privateKey: privateKey, commitTxImp: commitTxImp, completionHandler: { error, _ in
