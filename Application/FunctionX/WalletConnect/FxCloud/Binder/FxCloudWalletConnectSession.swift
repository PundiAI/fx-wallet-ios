...
class FxCloudWalletConnectSession: WalletConnectSession {
    ...
    private func bindPing() {
        isConnected.subscribe(onNext: { [weak self] v in
    ...
    override func accounts(for _: WCSessionRequestParam) -> [String] { return [] }
    override func handleMethod(_ request: JSON, _ method: String, _ parameter: JSON) {
        switch method {
        case "login":
            login(request, parameter: parameter)
        case "unjail":
            unjailValidator(request, parameter: parameter)
        case "create-validator":
            createValidator(request, parameter: parameter)
        case "sign-create-validator":
            signCreateValidator(request, parameter: parameter)
        case "get-node-validator-keyPair":
            getValidatorKeypair(request, parameter: parameter)
        case "get-validator-delegate-address":
            getDelegatorAddress(request, parameter: parameter)
        default: break
        }
    }

    private func login(_: JSON, parameter: JSON) {
        ...
        Router.pop(viewController, animated: false)
        DispatchQueue.main.async {
            authView.show(inView: Router.topViewController!.view)
        }
    }

    private func getValidatorKeypair(_: JSON, parameter: JSON) {
        let hrp = parameter["prefix"].string ?? "fx"
        let chainName = parameter["chainId"].string ?? "--"
        let validatorPKHrp = hrp + "valconspub"
        Router.pushToSubmitValidatorKeypair(wallet: wallet, hrp: hrp, chainName: chainName, parameter: parameter.dictionaryObject) { keypair in
    ...
    private func getDelegatorAddress(_: JSON, parameter: JSON) {
        let hrp = parameter["prefix"].string ?? "fx"
        let chainName = parameter["chainId"].string ?? "--"
        let validatorHrp = hrp + "valoper"
        Router.pushToSubmitValidatorAddress(wallet: wallet, hrp: hrp, chainName: chainName, parameter: parameter.dictionaryObject) { keypair in
    ...
    private func signCreateValidator(_: JSON, parameter: JSON) {
    ...
    private func createValidator(_: JSON, parameter: JSON) {
        guard let derivationPath = parameter["derivationPath"].string?.bip44Path else {
            error(.canceled, msg: "no derivationPath", data: [:])
            return
        }
        ...
        tx.coin = Coin(id: -2, type: 118, name: chainName, symbol: symbol, decimal: 18)
    ...
    private func unjailValidator(_: JSON, parameter: JSON) {
        guard let derivationPath = parameter["derivationPath"].string?.bip44Path else {
            error(.canceled, msg: "no derivationPath", data: [:])
            return
        }
        ...
        let tx = FxTransaction(parameter)
        tx.coin = Coin(id: -2, type: 118, name: parameter["chainId"].stringValue, symbol: parameter["denom"].stringValue, decimal: 18)
