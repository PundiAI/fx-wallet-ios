...
class SmsService {
    ...
    private(set) lazy var fetchName: APIAction<String> = {
        ...
        if let name = UserDefaults.standard.nameOnSMS(ofAddress: address) {
            ...
            s = fx.sms.name(ofAddress: address)
                .do(onNext: { [weak self] n in
                    self?.name = n
                    UserDefaults.standard.set(nameOnSMS: n, ofAddress: address)
                })
        }
        return APIAction(s)
    }()
    ...
    fileprivate func messageService(with receiver: SmsUser) -> SmsMessageService {
        ...
        if service == nil {
            service = SmsMessageService(receiver: receiver, wallet: wallet)
    ...
    func preload() -> Observable<[SMSChatListCellViewModel]> {
        ...
        return cache.selectAll(ofUser: address).map { [weak self] caches in
    ...
    private func filterMerge(_ users: [SmsUser], isFromCache: Bool = false) -> [SmsUser] {
        ...
        if !isFromCache, newContact.isNotEmpty {
            _ = cache.insertOrReplace(newContact, ofUser: address).subscribe()
        }
    ...
    private func fetchLatestSmsIfNeed() {
        ...
        fetchLatestSms.subscribe(onNext: { _ in
            welf?.sort(true)
        }).disposed(by: defaultBag)
    }
...
class SmsMessageService {
    class PageRecorder {
        ...
        init(_ id: String) {
            self.id = id
            minHeight = UserDefaults.standard.object(forKey: minHeightKey) as? UInt64
        }
    ...
    private func handle(items: [SMSChatCellViewModel]) {
        ...
        if updateMsgs.isNotEmpty {
            _ = cache.insertOrReplace(updateMsgs).subscribe()
        }
    }
    ...
    private func atomic(operation: () -> Void) {
        lock.wait()
        operation()
        lock.signal()
    }
    ...
    private func filterMerge(_ appendItems: [SMSChatCellViewModel], needSort: Bool = false) -> ([SMSChatCellViewModel], [SmsMessage], [SmsMessage]) {
        ...
        if needSort {
            appendItems = appendItems.sorted { $0.sortHeight <= $1.sortHeight }
        }
