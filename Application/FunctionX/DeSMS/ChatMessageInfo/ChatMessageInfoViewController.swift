//
//  ChatMessageInfoViewController.swift
//  XWallet
//
//  Created by HeiHua BaiHua on 2020/3/15.
...
class ChatMessageInfoViewController: WKTableViewController {
    ...
    init(receiver: SmsUser, wallet: FxWallet, sms: SmsMessage) {
        self.sms = sms
        self.service = SmsServiceManager.service(forWallet: wallet)
    ...
    private func bind() {
        ...
        items.push(Cell.self, m: CellViewModel(title: TR("Fee"), content: "0 \(FxChain.sms.token.uppercased())"))
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if vm.title == TR("MessageInfo.MessageHash") {
            path = "/#/sms/tx/\(vm.content.string)"
        } else if vm.title == TR("Height") {
            path = "/#/sms/block/\(vm.content.string)"
        } else if vm.title == TR("From") || vm.title == TR("To") {
            path = "/#/sms/account/\(vm.content.string)"
        }
...
//MARK: View
fileprivate func newTextLabel(_ text: String) -> UILabel {
...
fileprivate func newTitleLabel(_ text: String) -> UILabel {
...
extension ChatMessageInfoViewController {
    ...
    fileprivate class CellViewModel {
        ...
        init(title: String, name: String = "", content: String, contentIsLink: Bool = false) {
            ...
            self.height = 32 + 10 + nameHeight + contentHeight + 1
    ...
    fileprivate class Cell: WKTableViewCell {
        ...
        override func initSubView() {
            ...
            let padding = 18
            self.contentView.addSubviews([titleLabel, nameLabel, nameFxLabel, contentLabel])
