...
class NewChatViewController: WKViewController {
    ...
    init(wallet: FxWallet) {
        ...
        self.senderName = SmsServiceManager.service(forWallet: wallet).name
    ...
    private func sayHiToUser() {
        ...
        for user in service.latestContactList() {
            if user.rawValue.address == self.user.address {
                self.hud?.text(m: "User is already in the contact list")
                return
            }
        }
        ...
        weak var welf = self
        self.hud?.waiting()
        fx.sms.sendSms(toPK: self.user.pk, content: "Hi, i'm \(senderName)", fee: "1", gas: 80000).subscribe(onNext: { (result) in
            welf?.hud?.success(m: "")
