...
extension DappSelectAddressViewController {
    class DappCellViewModel: CellViewModel {
        ...
        init(_ wallet: Wallet, derivationAddress: Int, chain: FxChain.Types, token: String? = nil) {
            ...
            var name: String?
            if chain == .sms {
                name = UserDefaults.standard.nameOnSMS(ofAddress: address)
            } else {
                name = UserDefaults.standard.nameOnHUB(ofAddress: address)
            }
        ...
        fileprivate func save(name: String) {
            addressName.accept(name)
            if chain == .sms {
                UserDefaults.standard.set(nameOnSMS: name, ofAddress: address)
            } else {
                UserDefaults.standard.set(nameOnHUB: name, ofAddress: address)
            }
        }

        private func hanlder(result: JSON? = nil, error _: Error? = nil) {
