...
extension SelectAddressViewController {
    ...
    class ListViewModel: WKListViewModel<CellViewModel> {
        ...
        init(_ wallet: Wallet) {
            ...
            self.pager.hasNext = { $0.count < 100 }
...
extension SelectAddressViewController {
    ...
    class CellViewModel {
        ...
        init(_ wallet: Wallet, derivationAddress: Int) {
            self.wallet = wallet
            self.hdWallet = wallet.key.wallet(password: Data())
            self.derivationAddress = derivationAddress
            self.address = generateAddress()
            if let remark = UserDefaults.standard.remark(ofAddress: self.address) {
                self.addressRemark.accept(remark)
            }
        }
