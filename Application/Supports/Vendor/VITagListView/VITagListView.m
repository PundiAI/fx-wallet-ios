...
- (void)_initView
{
    ...
    UIMenuItem *deleteMenuItem = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Delete", @"") action:@selector(_deleteTagHandler:)];
    UIMenuItem *updateMenuItem = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"Update", @"") action:@selector(_updateTagHandler:)];
    self.menuController.menuItems = @[updateMenuItem,deleteMenuItem];
    ...
    self.tagInputField.placeholder = NSLocalizedString(@"", @"");
    ...
    [self.tagInputField onDeleteBackward:^{
        ...
        else
        {
            ...
            if (theView.tagChangeBlock != nil) {
                theView.tagChangeBlock(theView.tagArray);
            }
            theView.tagInputField.tagSelectedIndex = -1;
            [theView _reloadData];
            
            //隐藏菜单
            [theView.menuController setMenuVisible:NO animated:YES];
        }

    }];
    [self addSubview:self.tagInputField];
    
    self.tagMaxLength = 24;
    self.itemGap = 12;
    self.itemLeadGap = 24;
    ...
    self.lastLayoutPoint = CGPointMake(self.itemLeadGap, self.itemLeadGap);
...
- (void)setBlock:(ContentHeightBlock)block {
    _block = block;
}


- (void) reSetContentHeight:(CGFloat )height {
...
+ (CGFloat)staticHeight:(CGPoint)start width:(CGFloat)width tags:(NSMutableArray<NSString *> *)tagArray {
    __block CGFloat left = start.x;
    __block CGFloat top = start.y;
    [tagArray enumerateObjectsUsingBlock:^(NSString * _Nonnull tag, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *temp = [[NSString alloc] initWithFormat:@"%ld %@", idx + 1 , tag];
       CGSize tagSize = [temp sizeWithAttributes:@{NSFontAttributeName : [VITagView preferredTagFont]}];
        tagSize.width += 30;
        tagSize.height = 38;

        if (left + tagSize.width + 12 > width)
        {
            //换行
            left = 24;
            top += 38 + 12;
        }
        left += tagSize.width + 12;
    }];
     return top + 38 + 24;
}
...
- (void)_reloadData
{
    ...
    __block CGFloat left = self.itemLeadGap;
    __block CGFloat top = self.itemLeadGap;
...
- (void)_reloadPreData:(BOOL)endEdit
{
    ...
    CGFloat contentHeight = top + self.itemHeight + self.itemLeadGap + self.itemGap ;
    [self reSetContentHeight: contentHeight];
...
//    }
    ...
    self.startIndex = 0;
    self.endIndex = self.tagLayoutAttributesArray.count;
    ...
    for (NSInteger i = self.startIndex; i <= self.endIndex && i < self.tagLayoutAttributesArray.count; i++)
    {
        ...
        if (self.errorTags.count > 0 && [self.errorTags containsObject:self.tagArray[i]] ) {
            tagView.layer.borderColor = [UIColor colorWithRed:250 / 255.0
                                                        green:98 / 255.0
                                                         blue:55 / 255.0
                                                        alpha:1].CGColor;
            [tagView setValue:self.tagArray[i] isError:true];
        } else {
...
//    [self scrollRectToVisible:self.visibleRect animated:YES];
    ...
    CGFloat contentHeight = top + self.itemHeight + self.itemLeadGap;
    [self reSetContentHeight: contentHeight];
    self.contentSize = CGSizeMake(self.bounds.size.width, contentHeight);
    //    NSLog(@"contentSize %f   tagInputField.frame  %@", self.contentSize.height, NSStringFromCGRect(frame));
...
- (void)_deleteTagHandler:(id)sender
{
    if (self.tagInputField.tagSelectedIndex != -1)
    {
        ...
        if (self.tagChangeBlock != nil) {
            self.tagChangeBlock(self.tagArray);
        }
        self.tagInputField.tagSelectedIndex = -1;
        [self _reloadData];
    }
...
/**
 ...
 @param sender 事件对象
 */
- (void)_updateTagHandler:(id)sender
{
        if (self.tagInputField.tagSelectedIndex != -1)
        {
            NSNumber *index = @(self.tagInputField.tagSelectedIndex);
            VITagView *tagView = [self.visibleTagsDict objectForKey:index];
            if (self.tagEditBlock != nil ) {
                self.tagEditBlock(self, tagView);
            }
        }
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (self.tagArray.count >= _tagMaxLength) {
        return NO;
    }
    ...
    if ( [string  isEqual: @" "]) {
        //清空内容
        NSString *tag = [[textField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] stringByReplacingOccurrencesOfString:@" " withString:@""];
        if (tag.length == 0)
        {
            return NO;
        }
        ...
        [self.tagArray addObject:tag.lowercaseString];
        if (self.tagChangeBlock != nil) {
            self.tagChangeBlock(self.tagArray);
        }
        [self _reloadData];
