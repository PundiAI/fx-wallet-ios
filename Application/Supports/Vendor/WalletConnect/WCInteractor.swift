...
public typealias CustomRequestClosure = (_ topic: String, _ result: [String: Any]) -> Void
...
open class WCInteractor {
    ...
    public var didUpdateState: ((WCInteractorState) -> Void)?
    ...
    public var onSessionBeKilled: (() -> Void)?
    ...
    public init(session: WCSession, meta: WCPeerMeta, sessionRequestTimeout: TimeInterval = 20, clientId: String) {
...
extension WCInteractor {
    ...
    private func encryptAndSend(data: Data) -> Promise<Void> {
        WCLog("==> sent(rawValue): \(String(data: data, encoding: .utf8)!) ")
