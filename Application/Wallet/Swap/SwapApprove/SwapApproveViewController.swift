...
class SwapApproveViewController: WKViewController {
    ...
    private func bindList() {
        ...
        unfold.asDriver().drive(onNext: { (value) in
    ...
    override func router(event: String, context: [String : Any]) {
        if event == "Edit", let _ = (context[eventSender] as? InfoCell) {
            ...
            Router.pushToApproveEditPermission(wallet: self.wallet, vm: vm) { [weak self] (amount) in
    ...
    private func convertMoney(_ amount: String?) {
        ...
        self.vm?.amountTitle.accept(amountTitle)
    ...
    private func getPrice() {
        ...
        var amountTitle = "\(pow(2, 256.d).s) \(fromToken.symbol)"
        var subAmount = self.approveAmount
        if let approveAmount = self.approveAmount {
        ...
        weak var welf = self
        let tx:EthereumTransaction? = UniswapV2.Router02.approveAbi(accountAddress: fromAccount.address, token: fromToken.contract, maxAmount: subAmount)
        guard let txTran = tx else {
            return
        }
        
        self.hud?.waiting()
        UniswapV2.Router02.buildEthTx(txTran, fromCoin: fromToken, wallet: wallet.rawValue)
    ...
    func approved() {
        ...
        let tx:EthereumTransaction? = UniswapV2.Router02.approveAbi(accountAddress: fromAccount.address,
...
extension SwapApproveViewController {
    ...
    class ApproveViewModel { 
        lazy var amount = BehaviorRelay<String>(value: "")
        ...
        init(tx: FxTransaction) {
            ...
            let legalAmount = "$" + fee.div10(coin.feeDecimal).mul(exchangeRate.value).thousandth(ThisAPP.CurrencyDecimal)
            self.feeTitle.accept("\(_feeText) \(feeSymbol)")
            self.legalAmountTitle.accept(legalAmount)
        }
