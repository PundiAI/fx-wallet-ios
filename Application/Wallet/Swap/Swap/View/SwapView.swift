...
extension SwapViewController {
    class SwapStackView: AloeStackView {
        let touchBeganOberver = PublishSubject<Void>()
        override func touchesBegan(_: Set<UITouch>, with _: UIEvent?) { touchBeganOberver.onNext(())
        }
    }
    ...
    class View: UIView {
        ...
        private func layoutUI() {
            ...
            let offset = CGFloat(0.0.ifull(10.auto()))
        ...
        func showNotice(_ model: ApprovedModel) {
            ...
            addView(approveNotice)
            approveNotice.titleLabel.text = TR("Approve %@", model.token)
...
extension SwapViewController {
    class CoinView: UIView, UITextFieldDelegate {
        ...
        lazy var chooseTokenButton: UIButton = {
            ...
            v.titleLabel?.textColor = .white v.setBackgroundImage(UIImage.createImageWithColor(color: HDA(0x0552DC)), for: .normal)
            return v
        }()
        ...
        private func configuration() {
            ...
            chooseTokenButton.setTitle(TR("Choose token"), for: .normal)
            ...
            if filtered == string {
                if var newTextString = textField.text {
                    ...
                    if newTextString == numberDecimal.description {
                        ...
                        let dotsCount = newTextString.components(separatedBy: ".").count return (range.length == 0 && string == "0") && dotsCount == 2
                    }
                }
                return true
            } else {
    ...
    class PriceView: UIView {
        ...
        private func layoutUI() {
            ...
            refeshBtn.snp.makeConstraints { make in
                make.size.equalTo(CGSize(width: 24.auto(), height: 24.auto()))
                make.centerY.equalTo(titleLabel) make.right.equalToSuperview().offset(-38.auto())
            }
