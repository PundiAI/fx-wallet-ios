...
extension DappWebViewController {
    ...
    override open func webView(_: WKWebView, runJavaScriptAlertPanelWithMessage message: String, initiatedByFrame _: WKFrameInfo,
    ...
    override open func webView(_: WKWebView, runJavaScriptConfirmPanelWithMessage message: String, initiatedByFrame _: WKFrameInfo,
...
public class DappJSAction: WKJSAction {
    ...
    func pushToSelectAddressIfAllow(_: @escaping ((String, String, PrivateKey) -> Void)) {
        //        DispatchQueue.main.async {
        //
        ...
        DispatchQueue.main.async {
            Router.manager.showAuthorizeDappAlert(dapp: self.dapp, authorityTypes: [0]) { authVC, allow in
                authVC?.dismiss(animated: false, completion: {
                    ...
                    Router.manager.showDappSelectAddressAlert(wallet: self.wallet, dapp: self.dapp, chain: chain) { vc, _, _, _ in
...
public class DappJSSms: DappJSAction {
    ...
    @objc func transation(_ params: Any, _ callback: XWVScriptObject) {
        ...
        pushToSelectAddressIfAllow { address, _, privateKey in
...
public class DappJSNavigation: DappJSAction {
    ...
    @objc func router(_ params: Any, _ callback: XWVScriptObject) {
        guard let scene = string(forKey: "scene", in: params),
            let url = string(forKey: "url", in: params)
        else {
...
extension XWVScriptObject {
    fileprivate enum Code: Int {
        ...
        case noAddressToRegister = 300_005
        case initializationFailed = 300_006
