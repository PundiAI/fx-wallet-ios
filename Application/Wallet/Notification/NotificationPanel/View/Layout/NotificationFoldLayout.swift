//
//  XWFoldCollectionLayout.swift
//  XWFoldCollectionLayout
//
//  Created by Andy on 20/07/2020.
//  Copyright © 2020 Pundix. All rights reserved.
//
...
class NotificationFoldLayout: UICollectionViewFlowLayout {
    ...
    var contentHeight:CGFloat = NotificationPanelViewController.minContentHeight
...
extension NotificationFoldLayout {
    ...
    override func initialLayoutAttributesForAppearingItem(at itemIndexPath: IndexPath) -> UICollectionViewLayoutAttributes? { 
        if self.insertedIndexPaths?.contains(itemIndexPath) ?? false , itemIndexPath.section == 0 {
...
class NotificationHideLayout: UICollectionViewFlowLayout {
    var attributes0 = Array<UICollectionViewLayoutAttributes>()
    var attributes1 = Array<UICollectionViewLayoutAttributes>()
    var contentSize: CGSize = CGSize(width: 0, height: 0)
    private var delegate:NotificationLayoutDelegate!
    ...
    override func prepare() {
        ...
        let left = CGFloat(0.0)
        let width = collectionView?.frame.size.width ?? ScreenWidth
        let height = collectionView?.frame.size.height ?? ScreenHeight
        ...
        guard let count1 = collectionView?.numberOfItems(inSection: 1) else {
            return
        }
         
        for item in 0..<count1 {
            let indexPath = IndexPath(item: item, section: 1)
            let attribute = UICollectionViewLayoutAttributes(forCellWith: indexPath)
            let size = delegate.itemSize(layout: self, indexPath: indexPath)
            let frame = CGRect(x: left, y: -size.height, width: width, height: size.height)
...
extension NotificationHideLayout { 
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        var visibleLayoutAttributes = [UICollectionViewLayoutAttributes]()
