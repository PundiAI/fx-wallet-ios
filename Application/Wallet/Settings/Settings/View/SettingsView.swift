...
extension SettingsViewController {
    class View: UIView { lazy var navBar = FxBlurNavBar()
        var backButton: UIButton { navBar.backButton }
        fileprivate lazy var tableHeaderView = UIView(size: CGSize(width: ScreenWidth, height: 110.auto()))
        ...
        private func layoutUI() {
            ...
            let height = titleLabel.text?.height(ofWidth: ScreenWidth - 24.auto() * 2, attributes: [.font: XWallet.Font(ofSize: 40, weight: .bold)]) ?? 58
            tableHeaderView.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: 53.auto() + height.auto() + FullNavBarHeight)
            ...
            titleLabel.wk.adjust(frame: CGRect(x: 24.auto(), y: FullNavBarHeight + 8.auto(), width: 0, height: 58))
        }
    }
}
...
extension SettingsViewController {
    class HeroDebugCell: Cell {
        ...
        override func layoutUI() {
            super.layoutUI()
            titleLabel.text = TR("Hero Debug View") contentView.addSubview(switCh)
            switCh.snp.makeConstraints { make in
                make.centerY.equalToSuperview()
                make.right.equalTo(-24.auto())
            }
...
extension SettingsViewController {
    class TopHeaderCell: FxTableViewCell {
        override class func height(model _: Any?) -> CGFloat { return 28.auto() }
        lazy var titleLabel: UILabel = {
            let v = UILabel()
...
extension SettingsViewController {
    class SectionHeaderCell: TopHeaderCell {
        override class func height(model _: Any?) -> CGFloat { return 52.auto() }
        override func layoutUI() {
            super.layoutUI()
        }
    }
    ...
    class AssetCell: SingleCell {
        ...
        override class func height(model: Any?) -> CGFloat {
            if let value = model as? String, value.length > 0 {
                ...
                return (24 + 22 + 8).auto() + height + (24 + 8).auto()
            }
            return 88.auto()
        }
