...
class CheckBackUpViewController: WKViewController {
    ...
    private func bind() {
        ...
        let attri = NSMutableAttributedString(string: temp)
        attri.addAttributes([.font: XWallet.Font(ofSize: 24, weight: .bold)], range: NSRange(location: 0, length: temp.length))
        attri.addAttributes([.foregroundColor: COLOR.title], range: NSRange(location: 0, length: "\(viewModel.currentPage)".length))
        let bool = viewModel.currentPage == 3
        attri.addAttributes([.foregroundColor: bool ? COLOR.title : COLOR.title.withAlphaComponent(0.3)],
                            range: NSRange(location: "\(viewModel.currentPage)".length, length: temp.length - "\(viewModel.currentPage)".length))
...
extension CheckBackUpViewController {
    ...
    private func bindHero() { weak var welk = self
        let animator = WKHeroAnimator({ _ in
            ...
            welk?.wk.view.tagButtons.each { _, button in
