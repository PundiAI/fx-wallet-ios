...
extension TokenInfoAddressListBinder {
    class Cell: FxTableViewCell {
        ...
        override func bind(_ viewModel: Any?) {
            ...
            view.balanceLabel.wk.set(amount: vm.balance.value.value, symbol: coin.token, power: coin.decimal, thousandth: coin.decimal, autoTrim: false, animated: false)
            vm.balance.value.asDriver().drive(onNext: { (balance) in
                welf?.view.balanceLabel.wk.set(amount: balance, symbol: coin.token, power: coin.decimal, thousandth: coin.decimal, autoTrim: false)
            }).disposed(by: reuseBag)
        }
