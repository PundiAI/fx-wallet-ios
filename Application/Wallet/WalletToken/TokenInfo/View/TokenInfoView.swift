...
extension TokenInfoViewController {
    class View: UIView {
        ...
        func relayout(navBar: WKNavigationBar, showCorners: Bool) {
            
            if !showCorners {
                navBar.layer.mask = nil
                headerCell?.tokenInfoBackView.backgroundColor = HDA(0xF4F4F4)
            } else {
                ...
                headerCell?.tokenInfoBackView.backgroundColor = .white
            }
        }
    }
}
...
extension TokenInfoViewController {
    ...
    class HeaderCell: FxTableViewCell {
        ...
        private var tokenInfoHeight: CGFloat {
            ...
            let itemCount: CGFloat = showAssetIdBlock() ? 4 : 3
            let height = (24.0 * 2.0) + (18.0 * itemCount) + 8.0 * (itemCount - 1)
            return height.auto()
        }
        private lazy var buyButtonHeight:CGFloat = (36 + 8).auto()
        ...
        private lazy var assetItemView: InfoItemView = {
            ...
            v.textLabel.text = fxTextPlaceholder
            v.textLabel.lineBreakMode = .byTruncatingMiddle
            return v
        }()
        ...
        override func layoutUI() {
            ...
            legalBalanceLabel.snp.makeConstraints { (make) in
                make.top.equalTo(balanceLabel.snp.bottom).offset(6.auto())
                make.left.right.equalToSuperview().inset(24.auto())
            ...
            buyButton.snp.makeConstraints { (make) in
                ...
                make.top.equalTo(legalBalanceLabel.snp.bottom).offset(24.auto())
            }
            ...
            if showAsset {
                ...
                assetItemView.textLabel.snp.remakeConstraints { (make) in
                    make.right.equalToSuperview()
                    make.centerY.equalTo(assetItemView.titleLabel)
                    make.left.equalTo(assetItemView.titleLabel.snp.right).offset(8.auto())
                    make.height.equalTo(18.auto())
                }
            }
            //tokenInfoView...e
        ...
        func updatePriceItem(name: String, value: String) {
            ...
            if itemView != nil {
                itemView?.textLabel.text = "$ \(value.thousandth(ThisAPP.CurrencyDecimal, isLegal: true))"
            } else {
