...
class TokenInfoViewController: WKViewController {
    ...
    override func bindNavBar() {
        super.bindNavBar()
        navigationBar.screen(.iFull)?.setCorner(radius: 36.auto())
    ...
    private func bindHeader() {
        ...
        viewModel.balance.asDriver()
            .drive(onNext: {
                welf?.header.balanceLabel.wk.set(amount: $0, symbol: coin.token, power: coin.decimal, thousandth: ThisAPP.NumberDecimal)
            }).disposed(by: defaultBag)
        
        viewModel.legalBalance.value.asDriver()
            .drive(onNext: { welf?.header.legalBalanceLabel.wk.set(amount: $0, thousandth: ThisAPP.CurrencyDecimal, isLegal: true, mb: true) })
            .disposed(by: defaultBag)
    ...
    private func predisplay() {
        header.balanceLabel.wk.set(amount: viewModel.balance.value, symbol: coin.token, power: coin.decimal, thousandth: ThisAPP.NumberDecimal, animated: false)
        header.legalBalanceLabel.wk.set(amount: viewModel.legalBalance.value.value, thousandth: ThisAPP.CurrencyDecimal, isLegal: true, mb: true, animated: false)
    }
...
extension TokenInfoViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        wk.view.relayout(navBar: navigationBar, showCorners: listView.contentOffset.y >= (header.estimatedHeight - 40))
    }
...
extension TokenInfoViewController {
   ...
   private func bindHero() {
        ...
        animators["0"] = WKHeroAnimator({ (_) in 
            welf?.navigationBar.clipsToBounds = true
            welf?.navigationBar.hero.modifiers = [.fade,
                                                  .whenAppearing(.delay(0.20)),
                                                  .cornerRadius(36.auto()),
            ...
            welf?.header.aBackgroundView.hero.modifiers = [.whenDisappearing(.delay(0.20)),
                                                           .useGlobalCoordinateSpace]
            
            welf?.header.contentView.hero.modifiers = [.fade, .useGlobalCoordinateSpace]
            welf?.header.tokenIV.hero.id = "token_image_view"
            welf?.header.tokenInfoBackView.hero.modifiers = [.whenAppearing(.opacity(0), .delay(0.25)),
                                                             .whenDisappearing(.opacity(0)),
                                                             .useOptimizedSnapshot]
            welf?.header.tokenInfoContentView.hero.modifiers = [.whenDisappearing(.opacity(0)),
                                                                .whenAppearing(.opacity(0), .delay(0.20)),
                                                                .useOptimizedSnapshot, .useGlobalCoordinateSpace]
            welf?.header.balanceInfoView.hero.modifiers = [.whenDisappearing(.opacity(0)),
                                                           .whenAppearing(.opacity(0), .delay(0.20)),
                                                           .useOptimizedSnapshot, .useOptimizedSnapshot]
