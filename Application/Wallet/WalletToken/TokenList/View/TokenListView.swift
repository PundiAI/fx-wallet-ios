...
extension TokenListViewController {
    ...
    class TokenNavigationBar: WKNavigationBar {
        ...
        lazy var backgroundBlurView: VisualEffectView = {
            return VisualEffectView().then {
                $0.colorTint = HDA(0x080A32)
                $0.colorTintAlpha = 0.95 
                $0.blurRadius = 5
            }
        }()
        ...
        lazy var pindingButton: FxTxLoadingView = {
            let v = FxTxLoadingView.loading16().then {
                $0.ballColor = HDA(0x5B8FF9)
                $0.ringColor = UIColor.white
            }
            v.isHidden = true
        ...
        func layoutView() {
            let pindBoxView = UIView().then {
                $0.addSubview(pindingButton)
                pindingButton.loading()
                pindingButton.snp.makeConstraints { (make) in
                    make.centerY.equalToSuperview()
                    make.right.equalToSuperview()
                    make.size.equalTo(CGSize(width: 16.auto(), height: 16.auto()))
                }
            }
            pindBoxView.isUserInteractionEnabled = false
             
            addView(titleLabel, amountLabel, pindBoxView, notifButton, settingsButton)
            titleLabel.snp.makeConstraints { (make) in
                make.left.equalTo(24.auto())
                make.height.equalTo(22.auto())
                make.bottom.equalToSuperview().inset(3.auto())
            }
            
            settingsButton.snp.makeConstraints { (make) in
                make.centerY.equalTo(titleLabel.snp.centerY).offset(1.auto())
                make.right.equalToSuperview().inset(24.auto())
                make.size.equalTo(CGSize(width: 39.auto(), height: NavBarHeight))
            }
            ...
            pindBoxView.snp.makeConstraints { (make) in
                make.right.equalTo(notifButton.snp.right).offset(-42.auto())
                make.centerY.equalTo(notifButton.snp.centerY)
                make.size.equalTo(CGSize(width: 39.auto(), height: NavBarHeight))
            }
            ...
            XEvent.App.ApplicationDidBecomeActive.on {[weak self] _ in
                ...
                if enabled != this.isReduceTransparencyEnabled {
                    this.isReduceTransparencyEnabled = enabled
                    self?.backgroundBlurView.colorTint = HDA(0x080A32)
                    self?.backgroundBlurView.colorTintAlpha = 0.95
                    self?.backgroundBlurView.blurRadius = 5
                }
...
extension TokenListViewController {
    ...
    class View: UIView {
        var initLayoutAction:Action<Void,Void>?
        
        var amountLabelHeight:CGFloat = 58.auto()
        var tableFooterHeight:CGFloat = (32 + 56 + 100).auto()
        
        let navBarView = UIImageView().then {
            $0.screen(.iFull)?.addCorner([.topLeft, .topRight], radius: 36.auto())
        }
        ...
        lazy var addWalletButton: UIButton = {
            ...
            v.setTitlePosition(.right, withAdditionalSpacing: 30.auto())
        ...
        private func layoutUI() { 
            ...
            let padding:CGFloat = (NavBarHeight-20)/2.0
            listHeaderView.clipsToBounds = true 
            listHeaderView.size = CGSize(width: ScreenWidth, height: FullNavBarHeight + 156.auto() + 21.auto() - padding)
        ...
        func layoutNotif(_ show: Bool) {
            ...
            if !show, listView.contentInset.top > 0 {
                if let anim = POPSpringAnimation(propertyNamed: kPOPScrollViewContentInset) {
                    ...
                    listView.pop_add(anim1, forKey: "kPOPScrollViewContentOffset")
                    anim1?.completionBlock = { [weak listView] _,_ in 
                        DispatchQueue.main.async {
                            listView?.setContentOffset(CGPoint(x: 0, y: 0) , animated: false)
