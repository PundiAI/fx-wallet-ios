...
class AddTokenViewController: WKViewController, UITextFieldDelegate {
    ...
    private func add(coin: Coin) { Router.showAddCoinAlert(coin: coin) { [weak self] allow in
...
extension AddTokenViewController {
    ...
    private func bindHero() { weak var welf = self
        let navBarBackgoundColor = wk.view.navBar.backgroundColor
        let animator0 = WKHeroAnimator({ _ in
            ...
            welf?.wk.view.aBackgroundView.hero.modifiers = [.useOptimizedSnapshot, .useGlobalCoordinateSpace] welf?.wk.view.mainListView.hero.modifiers = [.translate(y: 1000), .fade, .useGlobalCoordinateSpace]
