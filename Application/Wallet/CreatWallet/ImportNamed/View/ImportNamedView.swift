...
extension ImportNamedViewController {
    class View: UIView {
        ...
        lazy var subtitleLabel: UILabel = {
            ...
            v.numberOfLines = 0
            v.backgroundColor = .clear
            let paraph = NSMutableParagraphStyle().then {
                $0.lineSpacing = 5
            } 
            let attributes = [NSAttributedString.Key.font: XWallet.Font(ofSize: 16),
                              NSAttributedString.Key.paragraphStyle: paraph]
            v.attributedText = NSAttributedString(string: TR("ImportNamed.SubTitle"), attributes: attributes)
            return v
        }()
        ...
        private func layoutUI() {
            ...
            tipLabel.snp.makeConstraints { (make) in
                make.top.equalTo(nickNameLabel.snp.bottom).offset(6.auto())
                make.left.right.equalToSuperview().inset(24.auto())
            }
