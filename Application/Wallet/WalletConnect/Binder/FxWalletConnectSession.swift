...
class FxWalletConnectSession {
    ...
    fileprivate func bind(interactor: WCInteractor) {
        ...
        interactor.onCustomRequest = { _, request in
    ...
    private func authorization(_: JSON, parameter: JSON) {
        let msg = Fx.AuthorizationMessage()
        msg.address = context.address
        ...
        response(data: ["sign": signedMsg])
    }

    private func encodeMsg(_: JSON, parameter: JSON) {
    ...
    private func register(_: JSON, parameter: JSON) {
