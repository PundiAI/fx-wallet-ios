...
class SelectOrAddAccountViewController: WKViewController {
    ...
    private func bindRightListView() {
        ...
        viewModel.itemCount.subscribe(onNext: { [weak self] value in
            self?.wk.view.rightListTitleLabel.text = "\(value) results"
...
extension SelectOrAddAccountViewController {
    override func heroAnimator(from: String, to: String) -> WKHeroAnimator? {
        ...
        case ("DappPageListViewController", "SelectOrAddAccountViewController"): return animators["0"]
        case ("SelectOrAddAccountViewController", "ReceiveTokenViewController"): return animators["1"]
        default: return nil
        }
    }

    private func bindHero() { weak var welf = self let animator0 = WKHeroAnimator({ _ in
        welf?.wk.view.headerBlur.alpha = 0
        welf?.wk.view.blurContainer.alpha = 0 welf?.wk.view.aBackgroundView.hero.id = "token_list_background"
